...
def search_include_at_least_one(detected_ingredient):
    ...
    search_results = recipe_df[recipe_df['CKG_MTRL_CN'].apply(lambda x: any(ingredient in str(x) for ingredient in detected_ingredient))]
...
def search_all_include(detected_ingredient):
    ...
    search_results = recipe_df[recipe_df['CKG_MTRL_CN'].apply(lambda x: all(ingredient in str(x) for ingredient in detected_ingredient))]
    return search_results