...
def camera():
    if st.session_state.camera_running:
        ...
        with col2:
            ...
            # camera_button_con.button("촬영 시작", use_container_width=True, on_click=start_camera)

            # 버튼을 동적으로 관리하기 위해 empty() 사용, 클릭하면 이 버튼 사라져야됨
            camera_button_placeholder = st.empty()
            camera_button_placeholder.button("촬영 시작", use_container_width=True, on_click=lambda: [start_camera(), camera_button_placeholder.empty()])
