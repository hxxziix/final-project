...
def get_user_recipe_df(recommend_df, user_recipe_number):
    """
    사용자가 사용한 레시피 번호를 기반으로 유저 레시피 DataFrame을 반환합니다.

    :param recommend_df: 전체 레시피 데이터프레임
    :param user_recipe_number: 사용자가 선택한 레시피 번호의 리스트
    :return: 유저 레시피 DataFrame
    """
    # 유저 레시피에 중복이 있으면 중복 제거
...
def user_recipe_recommend(user_recipe_number):
    ...
    # 유저 레시피 DataFrame 생성
    user_recipe_df = get_user_recipe_df(recommend_df, user_recipe_number)
...
# # 여러개 받지 않고 한개만
# def user_recipe_recommend_one(user_recipe_number):
#     # 레시피 데이터 가져오기
#     recommend_df = get_recommend_df()
...
#     # 유사도 점수 추출 (단일 레시피에 대한 유사도 점수)
#     similarity_scores = similarity_scores[0]

#     # 유사도에 따라 레시피 추천 (자기 자신 제외)
#     # 사용자 레시피의 인덱스 찾기
#     user_recipe_index = recommend_df[recommend_df['레시피번호'] == user_recipe_number].index[0]
    
#     # 자기 자신 제외
#     similarity_scores[user_recipe_index] = -1  # 자기 자신과의 유사도는 무시

#     # 유사도가 높은 상위 5개 레시피 추천
#     top_indices = similarity_scores.argsort()[::-1][:5]  # 유사도가 높은 상위 5개 인덱스
#     recommended_recipes = recommend_df.iloc[top_indices]
